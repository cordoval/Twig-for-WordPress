...
function templateTwig($wpTemplate) {
	global $wp_query;
	
	// Rebuilding objects here
	for($i = 0; $i < count($wp_query->posts); $i++):
		$wp_query->posts[$i]->title = $wp_query->posts[$i]->post_title;
		$wp_query->posts[$i]->excerpt = $wp_query->posts[$i]->post_excerpt;
		$wp_query->posts[$i]->format = get_post_format($wp_query->posts[$i]->ID);
		$wp_query->posts[$i]->status = $wp_query->posts[$i]->post_status;
		$wp_query->posts[$i]->permalink = get_permalink($wp_query->posts[$i]->ID);
		
		$wp_query->posts[$i]->author = new TWIG_Author($wp_query->posts[$i]->post_author);
		$wp_query->posts[$i]->categories = get_the_category($wp_query->posts[$i]->ID);
		$wp_query->posts[$i]->tags = get_the_tags($wp_query->posts[$i]->ID);
		
		// Switch this - only do it if we're in a single
		if($wp_query->posts[$i]->comment_count > 0):
			$wp_query->posts[$i]->comments = get_comments(
				array('post_id' => $wp_query->posts[$i]->ID)
			);
		else:
			$wp_query->posts[$i]->comments = false;
		endif;
	endfor;
	// End rebuilding of objects
	...
	$twig->addFunction('wp_head', new Twig_Function_Function('wp_head'));
	$twig->addFunction('wp_footer', new Twig_Function_Function('wp_footer'));
	...
	$twig->addFunction('language_attributes', new Twig_Function_Function('language_attributes'));
	...
	$twig->addFunction('edit_post_link', new Twig_Function_Function('edit_post_link'));
	$twig->addFunction('is_front_page', new Twig_Function_Function('is_front_page'));
	...
	$twig->addFunction('rewind_posts', new Twig_Function_Function('rewind_posts'));
	...
	echo $template->render(array('bloginfo' => new TWIG_Blog_Info(), 'posts' => $wp_query->posts));
	exit;
}
add_action('template_include', 'templateTwig');